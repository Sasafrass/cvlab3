function [fig]=Lucas_Canede(img_a_path,img_b_path,window_size_input)  
    ...
    % vector
    for i = 1:window_size-1:size(gx,1)-window_size;
       counter2=1;
       for j = 1:window_size-1:size(gy,2)-window_size;
    ...
    %plot image
    [m, n] = size(img_1);
    arrow_location = 1:window_size-1:m-window_size;
    arrow_location=arrow_location+floor(window_size/2);
    arrow_location_X = 1:window_size-1:n-window_size;
    arrow_location_X=arrow_location_X+floor(window_size/2);
    figure();
    imshow(img_a);
    hold on;
    quiver(arrow_location_X, arrow_location, v_1,v_2, 'y');
end